 ...
 */
const SPEEDGUARD_VERSION = '2.0.2';
// If this file is called directly, abort.
if ( !defined( 'WPINC' ) ) {
    die;
}
if ( function_exists( 'speedguard_fs' ) ) {
    speedguard_fs()->set_basename( false, __FILE__ );
} else {
    ...
    if ( !function_exists( 'speedguard_fs' ) ) {
        ...
        if ( !function_exists( 'speedguard_fs' ) ) {
            ...
            speedguard_fs()->add_filter(
                'connect_message',
                'speedguard_fs_custom_connect_message_on_update',
                10,
                6
            );
...
><div id="szpic" style="background-image: url(' . $sz_image_url . '); background-size: cover; height: 75px; width: 75px; border-radius: 50%; overflow: hidden; float: right; margin: 1em;"></div></a>';
                ...
                return $picture . sprintf(
                    '<p>' . __( 'Hi there!' ) . '</p>' . '<p>' . __( 'My name is Sabrina.' ) . '</p>' . '<p>' . __( 'Please help me improve %2$s!' ) . '<br>' . __( 'I would like to make this plugin more compatible with the sites like yours, and make it more useful.' ) . '<br>' . __( 'If you opt-in, some basic WordPress environment info will be shared.' ) . '<br>' . __( 'No guarantee, but I might also send you email for security & feature updates, educational content, and occasional offers.' ) . '</p>' . '<p>' . __( 'If you skip this, that\'s okay! %2$s will still work just fine.', 'speedguard' ) . '</p>',
                    $user_first_name,
                    '<b>' . $plugin_title . '</b>',
                    '<b>' . $user_login . '</b>',
                    $site_link,
