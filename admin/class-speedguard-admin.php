...
class SpeedGuard_Admin {
    ...
    public function __construct( $plugin_name, $version ) {
        ...
        add_action( (( defined( 'SPEEDGUARD_MU_NETWORK' ) ? 'network_' : '' )) . 'admin_menu', [$this, 'speedguard_admin_menu'] );
        add_action( (( defined( 'SPEEDGUARD_MU_NETWORK' ) ? 'network_' : '' )) . 'admin_notices', [$this, 'show_admin_notices'] );
        ...
        if ( !is_plugin_active_for_network( 'speedguard/speedguard.php' ) || is_plugin_active_for_network( 'speedguard/speedguard.php' ) && is_main_site( $blog_id ) ) {
            ...
            add_action(
                ...
                [$this, 'guarded_page_unpublished_hook'],
                10,
                3
            );
            add_action(
                'before_delete_post',
                [$this, 'before_delete_test_hook'],
                10,
                1
            );
            ...
            add_filter( (( defined( 'SPEEDGUARD_MU_NETWORK' ) ? 'network_admin_' : '' )) . 'plugin_action_links_speedguard/speedguard.php', [$this, 'speedguard_actions_links'] );
        }
        ...
        add_action(
            'deleted_post',
            [$this, 'update_average_psi_on_deletion'],
            10,
            1
        );
    }
    ...
    public static function is_screen( $screens ) {
        ...
        $screens = str_replace( ['tests', 'settings', 'clients'], ['toplevel_page_speedguard_tests', 'speedguard_page_speedguard_settings', 'speedguard_page_speedguard_clients'], $screens );
    ...
    function check_tests_progress_fn() {
        ...
        if ( $current_tests_array ) {
            ...
            if ( !$current_tests_array && !$test_in_progress && $last_test_is_done_tr !== false ) {
                //if there are no tests in the queue, but last test has just completed
                delete_transient( 'speedguard_last_test_is_done' );
                $response = $last_test_complete;
            } else {
                // if there are no tests and not waiting for the last one to complete
                $response = $do_nothing;
            }
        }
    ...
    function mark_test_as_done_fn() {
        ...
        $test_result_data_from_post = wp_unslash( $_POST['test_result_data'] );
        // don't know where those slashes come from
        ...
        if ( !is_array( $current_tests_array ) || count( $current_tests_array ) < 1 ) {
            ...
            $both_devices_values_origin = [
                ...
                'desktop' => [
                    'cwv' => [
                        'lcp'      => $desktop_data['originCWV']['lcp'],
            ...
            if ( !is_array( $both_devices_values_origin['desktop']['cwv']['lcp'] ) && !is_array( $both_devices_values_origin['mobile']['cwv']['lcp'] ) ) {
    ...
    public function enqueue_styles() {
        if ( is_admin_bar_showing() && self::is_screen( 'dashboard,settings,tests' ) ) {
            wp_enqueue_style(
                $this->plugin_name,
                plugin_dir_url( __FILE__ ) . 'assets/css/speedguard-admin.css',
                [],
                $this->version
            );
        }
        if ( is_admin_bar_showing() && self::is_screen( 'tests' ) ) {
            wp_enqueue_style(
                $this->plugin_name . '-awesompletecss',
                plugin_dir_url( __FILE__ ) . 'assets/awesomplete/awesomplete.css',
                [],
                $this->version
            );
        }
    ...
    public function enqueue_scripts() {
        if ( is_admin_bar_showing() && self::is_screen( 'dashboard,settings,tests,plugins,clients' ) ) {
            ...
            wp_enqueue_script(
                $this->plugin_name,
                plugin_dir_url( __FILE__ ) . 'assets/js/speedguard-admin.js',
                [],
                $this->version,
                false
            );
            ...
            wp_enqueue_script(
                ...
                [],
                filemtime( plugin_dir_path( __FILE__ ) . 'assets/js/execute_tests.js' ),
                true
            );
            ...
            wp_enqueue_script(
                ...
                [],
                filemtime( plugin_dir_path( __FILE__ ) . 'assets/js/initiate_tests.js' ),
                true
            );
        ...
        if ( is_admin_bar_showing() && self::is_screen( 'tests' ) ) {
            ...
            wp_enqueue_script(
                $this->plugin_name . '-awesompletejs',
                plugin_dir_url( __FILE__ ) . 'assets/awesomplete/awesomplete.js',
                [],
                $this->version,
                true
            );
            wp_enqueue_script(
                'speedguardsearch',
                plugin_dir_url( __FILE__ ) . 'assets/js/speedguard-search.js',
                [$this->plugin_name . '-awesompletejs'],
                $this->version,
                true
            );
    ...
    function speedguard_admin_menu() {
        ...
        $this->tests_page_hook = add_submenu_page(
            'speedguard_tests',
            __( 'Speed Tests', 'speedguard' ),
            __( 'Speed Tests', 'speedguard' ),
            'manage_options',
            'speedguard_tests'
        );
        $this->settings_page_hook = add_submenu_page(
            ...
            'manage_options',
            'speedguard_settings',
            ['SpeedGuard_Settings', 'settings_page_function']
        );
    }
